...
function autoLimitOrders(market,assetKey){
  ...
  return [...orderWatchState.records.entries()].filter(([,r])=>r.assetKey===assetKey&&r.vault===market.vaultAddress&&r.maturity===market.maturityDateUnixTs&&market.orderbookAddresses?.includes(r.book)&&walletMonitor.wallets.has(r.owner)&&r.expiry>Date.now()/1000&&r.maturity>Date.now()/1000)
    .map(([key,r])=>({key,r,apy:buyOrderApy(r.rawPrice),stale:r.status==='Stale'||!r.checkedAt||Date.now()-r.checkedAt>12000}))
    .filter(o=>o.apy!==null).sort((a,b)=>b.r.rawPrice-a.r.rawPrice||a.key.localeCompare(b.key));
}
...
renderLimitCells=function(row,assetKey,market){
  ...
  row.limitGap.textContent=orders.length?orders.map(o=>{
    ...
    const gap=market.impliedApy*100-o.apy;return (gap>=0?'+':'')+gap.toFixed(2)+(o.stale||apyStale?' *':'');
  }).join('\n'):'—';
...
function evaluateAutoLimitAlerts(fromScan=false){
  ...
  for(const [assetKey,asset] of Object.entries(ASSETS)){
    ...
    for(const {key,r,apy,stale} of autoLimitOrders(market,assetKey)){
      if(stale)continue;
      ...
      if(threshold!==null&&Number.isFinite(market.impliedApy))check('gap',limitGapAtOrBelow(market.impliedApy*100,apy,threshold),'Gap ≤ '+threshold+' pp');
      ...
      const alarmKey='auto-limit:'+key,message=`${asset.label} · ${r.owner.slice(0,6)}…${r.owner.slice(-4)} · ${apy.toFixed(2)}% · ${reasons.join(' · ')} · ${apyDate(r.maturity*1000)}`;
