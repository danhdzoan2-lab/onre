...
assert.equal(run(`farthestApyMarket(fixtures,'mint',${now}).vaultAddress`),'later');
assert.equal(run(`farthestApyMarket(fixtures,'mint',${now+100}).vaultAddress`),'later');
assert.equal(run(`farthestApyMarket(fixtures,'mint',${now+200})`),null);
assert.equal(run(`farthestApyMarket(fixtures.filter(m=>m.vaultAddress!=='later'),'mint',${now}).vaultAddress`),'a');
ctx.inactive = {...market('inactive',now+1000),marketStatus:'inactive'};
assert.equal(run(`farthestApyMarket([...fixtures,inactive],'mint',${now}).vaultAddress`),'later');
...
(async()=>{
  ...
  assert.match(run('apyState.error'),/timed out/);
  pending=null;response={ok:true,status:200,json:async()=>[]};await run('fetchApy()');
  assert.equal(row.children[1].textContent,'No active maturity');
