...
vm.createContext(context);for(const file of ['position-groups.js','buy-orderbook.js'])vm.runInContext(fs.readFileSync(require('node:path').join(__dirname,'..',file),'utf8'),context);
...
(async()=>{
 ...
 respond();await flush();assert.equal(detail.children[2].textContent,'No open buy orders');
 assert.equal(detail.children[1].children[1].textContent,'Show all','Buy Orderbook defaults to My levels');
 detail.children[1].children[1].events.click();assert.equal(detail.children[1].children[1].textContent,'My levels');
 ...
 assert.match(detail.children[2].textContent,/Stale.*Rate limited/);tick();assert.equal(calls,2,'Retry-After backoff');
 market.vaultAddress='next-vault';tick();assert.equal(detail.open,false,'rollover closes old market');assert.equal(detail.children[3].children.length,0);
 assert.equal(calls,2);detail.open=true;tick();assert.equal(calls,3,'new maturity can load independently');timer();await flush();assert.match(detail.children[2].textContent,/timed out/);
