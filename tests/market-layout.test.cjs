...
buy.open=true;assert.ok(buy.open&&!sell.open,'Books expand independently');
layout.sync();assert.ok(buy.open&&!sell.open,'Refresh preserves disclosure state');
...
token.details.open=false;assert.equal(buy.open,true);assert.equal(sell.open,false);assert.equal(sim.open,false);
