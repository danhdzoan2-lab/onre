...
(async()=>{
  ...
  context.buyQueuePosition=()=>null;
  context.orderWatchRecord=o=>({...record,signature:o.tx_signature});
  context.orders=[{order_type:'buyYT',offer_idx:2,user_address:'o',price_implied_apy:65000,amount_remaining:'100',tx_signature:record.signature,created_at:new Date(101000).toISOString()}];
  tx.blockTime=100;
  context.market={vaultAddress:'v',maturityDateUnixTs:1000};
  context.snapshot={slot:901,book:{vault:'v',maturity:1000,prices:[{id:1,price:65000}],offers:new Map([[2,{owner:'o',side:2,virtual:0,pricePointer:1,created:100,expiry:200,amount:100n}]])}};
  context.snapshots=new Map([['b',context.snapshot]]);context.orders[0].orderbook_address='b';
