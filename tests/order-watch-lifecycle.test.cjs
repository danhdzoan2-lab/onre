...
function fixture(storage){
 ...
 for(const file of ['limit-monitor.js','reward-range.js','position-groups.js','order-watch.js','wallet-monitor.js'])vm.runInContext(fs.readFileSync(require('path').join(__dirname,'..',file),'utf8'),ctx);
