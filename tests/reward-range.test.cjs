...
assert.equal(run(`rewardPosition(band,String(band.low))`),'Trong range');
assert.equal(run(`rewardPosition(band,String(band.high))`),'Trong range');
assert.equal(run(`rewardPosition(band,'8.37')`),'Dưới range');
assert.equal(run(`rewardPosition(band,'9.15')`),'Trên range');
assert.equal(run(`rewardPosition(band,'')`),'Chưa nhập My APY');
...
assert.match(run('rewardRangeState.error'),/quá chậm/);assert.equal(run('rewardRangeState.inFlight'),false);
