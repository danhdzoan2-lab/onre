...
const focused=sellVisibleGroups([...groups,otherGroup],false,order=>order.id===1);
assert.equal(focused.length,1);assert.deepEqual(focused[0].rows,groups[0].rows,'My levels includes every competing order through the personal level');
